...
def visualize_node(data,node,nodename,**plot_para):
    ...
    if node.key == ('leaf',) and node_data.shape[0] <= 20 :
        ...
        # markers = node_data.columns
        # print('clustering',list(all_clustering.keys())[0].type())
        ...
        for i in range(len(markers)):
            ...
            plt.hist(X, bins=50, density = True, color = "lightblue")
...
def visualize_pair(data,node,nodename,**plot_para):
    ...
    for i in range(len(marker_pairs)):
        ...
        X1 = node_data.loc[current_indices, marker1]
        X2 = node_data.loc[current_indices, marker2]
...
def visualize_tree(root,data,outpath,filename,compact=False):
    ...
    # sc.pp.scale(data, max_value=10)
    sc.tl.pca(data,n_comps=40)
    data = pd.DataFrame(data=data.obsm['X_pca'],index=data.obs_names,columns=['PC'+str(i) for i in range(data.obsm['X_pca'].shape[1])])
